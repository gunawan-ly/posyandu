...
- `src/modules/` — **struktur per-modul** (fondasi multi-posyandu): tiap modul punya `views/` (list/form/detail), `db.ts` (lapisan data modul), `routes.ts` (rute + meta guard). Saat ini **modul `balita`** aktif; modul lain (bumil, remaja, dewasa & lansia) menyusul dengan menyalin *shape* folder ini — cukup tambah `...<modul>Routes` di router tanpa mengubah struktur inti. Jangan menumpuk service/views modul baru di `src/views/` atau file `db.ts` app-level — taruh di `src/modules/<modul>/`.
