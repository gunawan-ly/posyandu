...
- `npm test`                           # vitest run (kalkulator + status/bumil + render; baseline 91 hijau)
...
- `src/components/` — `KurvaWHO.vue` (kurva WHO self-draw + titik z-score; mode `bbu`/`tbu`/`bbtb`/`lika`/`lila`), `InputDusun.vue` (toggle segmen Dusun gaya pemilihan jenis kelamin — opsi Kayumas/Tengah/Cempaka; nilai lama di luar opsi tetap tersimpan dengan catatan kecil, sejak v2.30.0 dipakai ketiga modal identitas), `StatusBadge.vue` (label lengkap saja, tanpa kode), `Skeleton.vue` (blok loading animate-pulse untuk halaman fetch data), `AppNavbar.vue` (brand statis "Posyandu Wapalo" **bold**, tautan Beranda/Tentang/Dashboard + dropdown modul **Data Balita** (label dinamis: "Balita" di `/balita*`, "Bumil" di `/bumil*`, default "Data Balita"; tersedia desktop & mobile dalam sheet; menu desktop absolutely positioned agar layout stabil), tab aktif benar utk rute & anchor `#tentang` via IntersectionObserver, frosted glass), `AppFooter.vue` (info © + credit "Dikembangkan oleh Awan" di baris paling bawah; `mt-auto` → sticky di dasar viewport via layout `flex min-h-screen flex-col` di `App.vue`), `Reveal.vue`, `ui/` (komponen shadcn-vue; `Card` punya varian `solid`/`glass`/`glass-strong`)
