...
- ✅ Modul **Remaja** (`7–18 th`, fase 1 = identitas + CRUD, v2.39.0): daftar & cari, form baru/edit via modal (`FormModalRemaja`), halaman detail identitas + tombol Ubah/Hapus; lapisan data `listRemaja`/`ambilRemaja`/`buatRemaja`/`ubahRemaja`/`hapusRemaja`; tabel `remaja_identitas` (kolom sama balita/apras minus `bb_lahir`/`pb_lahir`/`sasaran` + `nama_sekolah`) + RLS kader baca/admin tulis + trigger `set_dibuat_oleh()`; **tanpa kunjungan/rekap/z-score** (standar antropometri & tabel kunjungan remaja menyusul). Modul **Dewasa/Lansia** masih placeholder: rute `/lansia*` terdaftar di router dengan guard `requiresAuth` (tambah butuh `requiresAdmin`); detail/perbarui dialihkan ke daftar; halaman memakai `PlaceholderModul.vue`. Aktivasi pencatatan penuh menyusul.
