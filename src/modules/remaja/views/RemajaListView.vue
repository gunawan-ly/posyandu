...
import FormModalRemaja from '@/modules/remaja/views/FormModalRemaja.vue'
import { listRemaja, hapusRemaja, type Remaja } from '@/modules/remaja/db'
...
} = useDaftarModul<Remaja>({
  kunciView: 'view-remaja',
  muat: listRemaja,
  hapus: hapusRemaja,
  namaItem: 'remaja',
})

// CTA dengan tautan /remaja?tambah=1 (mis. Dashboard) langsung membuka form tambah.
...
async function hapus(anak: Remaja) {
  const ok = await dlgHapus.value?.buka(
    `Hapus data ${anak.nama}?`,
    'Hapus Remaja',
    { merah: true },
  )
  if (!ok) return
...
<template>
  <div class="flex flex-col">
    ...
    <section class="mx-auto w-full max-w-6xl px-4 py-12 sm:px-6">
      <div class="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 class="font-display mt-3 text-3xl leading-tight sm:text-4xl">Daftar Remaja</h1>
          <p class="text-muted-foreground mt-3 max-w-xl text-sm">
            Anak usia sekolah & remaja 7–18 tahun. Kelola identitas sebagai dasar pencatatan
            tumbuh kembang.
          </p>
        </div>
      ...
      <div v-else-if="daftar.length === 0" class="mt-8">
        ...
        >
          ...
          <p class="font-display mt-4 text-lg">
            {{ cari ? 'Tidak ada remaja ditemukan' : 'Belum ada data remaja' }}
          </p>
          <p class="text-muted-foreground mt-1 max-w-sm text-sm">
            ...
            <template v-else>
              Mulai dengan menambahkan remaja pertama melalui tombol "Tambah Remaja".
            </template>
          </p>
      ...
      <div v-else-if="modeView === 'grid'" class="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card v-for="r in daftar" :key="r.id" class="h-full">
          <CardContent class="flex flex-col gap-3">
            <div class="flex items-start justify-between gap-4">
              <div class="flex min-w-0 items-center gap-3">
                ...
                <div class="min-w-0">
                  ...
                  <p class="text-muted-foreground mt-0.5 text-xs">
                    {{ labelJk(r.jenis_kelamin) }} · {{ formatUmur(r.tanggal_lahir) }}
                  </p>
                </div>
              </div>
            ...
            <div class="border-border/60 grid grid-cols-2 gap-2 border-t pt-3 text-xs">
              <div>
                <p class="text-muted-foreground font-bold uppercase">Sekolah</p>
                <p class="mt-0.5 truncate font-medium">{{ r.nama_sekolah || '—' }}</p>
              </div>
              <div>
                <p class="text-muted-foreground font-bold uppercase">Dusun</p>
                <p class="mt-0.5 truncate font-medium">{{ r.dusun || '—' }}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <div class="overflow-x-auto">
            <table class="w-full min-w-[1700px] text-sm">
              ...
              <tbody>
                <tr v-for="r in daftar" :key="r.id" class="group border-border/60 hover:bg-emerald-50/40 relative border-b last:border-0">
                  <td
                    class="bg-white group-hover:bg-emerald-50/40 py-3 pr-3 font-medium whitespace-nowrap sticky left-0 z-10"
                  >
                    <RouterLink :to="`/remaja/${r.id}`" class="hover:text-primary font-bold">
                      {{ r.nama }}
                    </RouterLink>
                  ...
                  <td class="py-3 pr-3 whitespace-nowrap">{{ labelJk(r.jenis_kelamin) }}</td>
                  <td class="text-muted-foreground py-3 pr-3 whitespace-nowrap">{{ formatUmur(r.tanggal_lahir) }}</td>
                  <td class="py-3 pr-3 break-all whitespace-nowrap">{{ r.nik || '—' }}</td>
                  <td class="max-w-[140px] truncate py-3 pr-3 whitespace-nowrap" :title="r.tempat_lahir || ''">{{ r.tempat_lahir || '—' }}</td>
                  <td class="text-muted-foreground py-3 pr-3 whitespace-nowrap">{{ formatTanggal(r.tanggal_lahir) }}</td>
                  <td class="max-w-[160px] truncate py-3 pr-3 whitespace-nowrap" :title="r.nama_orang_tua || ''">{{ r.nama_orang_tua || '—' }}</td>
                  <td class="py-3 pr-3 break-all whitespace-nowrap">{{ r.nik_orang_tua || '—' }}</td>
                  <td class="py-3 pr-3 break-all whitespace-nowrap">{{ r.nomor_kk || '—' }}</td>
    ...
    <FormModalRemaja v-model:open="modalTambah" @tersimpan="muat" />
    <FormModalRemaja v-model:open="modalUbahOpen" :remaja="modalUbahData" @tersimpan="muat" @update:open="(v) => { if (!v) { modalUbahOpen = false; modalUbahData = null } }" />
  </div>
</template>
