...
const judul = ref('Tambah Remaja baru')

watch(() => props.open, async (terbuka) => {
  if (!terbuka) return
  bersihkanForm()
  if (props.remaja) {
    judul.value = 'Ubah data Remaja'
    memuat.value = true
    try {
      const b = await ambilRemaja(props.remaja.id)
      if (b) isiForm(b)
    } catch (e) {
    ...
    judul.value = 'Tambah Remaja baru'
  }
})
...
async function simpan() {
  ...
  if (!nama.value.trim()) {
    pesanError.value = 'Nama remaja wajib diisi.'
    return
  }
  ...
  try {
    if (props.remaja) {
      await ubahRemaja(props.remaja.id, payload)
    } else {
      await buatRemaja(payload)
    }
    ...
    >
      ...
      <div class="max-h-[80vh] overflow-y-auto px-6 py-4">
        ...
        <form v-else class="space-y-5" @submit.prevent="simpan">
          <div>
            <label for="fm-nama" class="text-muted-foreground mb-1.5 block text-xs font-bold">Nama Remaja *</label>
            <input id="fm-nama" v-model="nama" type="text" :class="klsInput" placeholder="cth: Putri Ramadhani" />
          </div>
          ...
          <div>
            <label for="fm-sekolah" class="text-muted-foreground mb-1.5 block text-xs font-bold">Nama Sekolah</label>
            <input id="fm-sekolah" v-model="namaSekolah" type="text" :class="klsInput" placeholder="cth: SMPN 1 Wapalo" />
          </div>
          ...
          <div class="flex flex-wrap justify-end gap-2">
            ...
            <Button size="lg" type="submit" :disabled="sibuk">
              {{ sibuk ? 'Menyimpan…' : remaja ? 'Simpan Perubahan' : 'Simpan Remaja' }}
            </Button>
          </div>
        </form>
      </div>
