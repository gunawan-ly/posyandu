...
const judul = ref('Tambah balita baru')

watch(() => props.open, async (terbuka) => {
  if (!terbuka) return
  bersihkanForm()
  if (props.balita) {
    judul.value = 'Ubah data balita'
    memuat.value = true
    try {
      const b = await ambilBalita(props.balita.id)
      if (b) isiForm(b)
    } catch (e) {
    ...
    judul.value = 'Tambah balita baru'
  }
})
...
async function simpan() {
  ...
  try {
    if (props.balita) {
      await ubahBalita(props.balita.id, payload)
    } else {
    ...
    >
      ...
      <div class="max-h-[80vh] overflow-y-auto px-6 py-4">
        ...
        <form v-else class="space-y-5" @submit.prevent="simpan">
          <div>
            <label for="fm-nama" class="text-muted-foreground mb-1.5 block text-xs font-bold">Nama balita *</label>
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              <label for="fm-bb-lahir" class="text-muted-foreground mb-1.5 block text-xs font-bold">Berat lahir (kg)</label>
              <input id="fm-bb-lahir" v-model="bbLahir" type="number" inputmode="decimal" step="0.01" min="0" :class="klsInput" />
            </div>
            <div>
              <label for="fm-pb-lahir" class="text-muted-foreground mb-1.5 block text-xs font-bold">Panjang lahir (cm)</label>
              <input id="fm-pb-lahir" v-model="pbLahir" type="number" inputmode="decimal" step="0.1" min="0" :class="klsInput" />
            </div>
          </div>
          ...
          <div class="flex flex-wrap justify-end gap-2">
            ...
            <Button size="lg" type="submit" :disabled="sibuk">
              {{ sibuk ? 'Menyimpan…' : balita ? 'Simpan Perubahan' : 'Simpan Balita' }}
            </Button>
          </div>
        </form>
      </div>
